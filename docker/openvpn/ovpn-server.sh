...
function listClient() {
    ...
    echo "List of Users"
	echo ""
    CLIENT_LIST=$(tail -n +2 /etc/openvpn/easy-rsa/pki/index.txt | grep "^V" | cut -d '=' -f 2 | nl -s ') ')
    echo "$CLIENT_LIST" | pr -t -${COLUMNS}
	echo ""
}
...
function disableClient() {
	...
	echo ""
	echo "Select the existing client certificate you want to DISABLE"
    CLIENT_LIST=$(tail -n +2 /etc/openvpn/easy-rsa/pki/index.txt | grep "^V" | cut -d '=' -f 2 | nl -s ') ')
    echo "$CLIENT_LIST" | pr -t -${COLUMNS}	
...
function renewClientCert() {
  echo ""
  read -rp "Enter the client name to renew: " CLIENT_NAME
  cd /etc/openvpn/easy-rsa/ || exit 1

  CRT_PATH="/etc/openvpn/easy-rsa/pki/issued/${CLIENT_NAME}.crt"
  KEY_PATH="/etc/openvpn/easy-rsa/pki/private/${CLIENT_NAME}.key"
  CA_PATH="/etc/openvpn/easy-rsa/pki/ca.crt"
  TLS_CRYPT_KEY="/etc/openvpn/tls-crypt.key"
  OUTPUT_OVPN="/root/${CLIENT_NAME}.ovpn"

  if [[ ! -f "$CRT_PATH" ]]; then
    echo "❌ Client certificate not found: $CRT_PATH"
    return 1
  fi

  ./easyrsa renew "$CLIENT_NAME"
  echo ""

  cat > "$OUTPUT_OVPN" <<EOF
client
proto udp
remote opt.miiniigo.com 443
dev tun
resolv-retry infinite
nobind
persist-key
persist-tun
remote-cert-tls server
verify-x509-name server_n1L14Uc8Ka6SP7Vp name
auth SHA256
auth-nocache
cipher AES-128-GCM
tls-client
...
EOF
  ...
  echo "✅ Certificate renewed and .ovpn file updated: $OUTPUT_OVPN"
}
